...
class Gate
{
   friend void reportEverything(const SatSolver&);
public:
...
void reportEverything(const SatSolver& solver)
{
   for (size_t i = 0, n = gates.size(); i < n; ++i)
      cout << gates[i]->_gid << " " << solver.getValue(gates[i]->getVar()) << endl;   
}
...
int main()
{
   ...
   solver.addXorCNF(newV, gates[4]->getVar(), false, gates[5]->getVar(), false);
   solver.assumeRelease();  // Clear assumptions
   solver.assumeProperty(newV, true);  // k = 1
   result = solver.assumpSolve();
   reportEverything(solver);
